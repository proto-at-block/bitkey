provider "datadog" {
  api_key  = local.enable_datadog ? data.aws_secretsmanager_secret_version.dd-api-key[0].secret_string : "dummy-key-datadog-disabled"
  validate = false
}
...
resource "aws_ssm_parameter" "dd-api-key" {
  ...
  type  = "SecureString"
  value = data.aws_secretsmanager_secret_version.dd-api-key[0].secret_string
}
...
module "dd-logs-forwarder" {
  ...
  datadog_api_key_parameter = aws_ssm_parameter.dd-api-key[0].name
  depends_on = [
    resource.aws_ssm_parameter.dd-api-key
  ]
}
...
data "aws_secretsmanager_secret_version" "dd-api-key" {
  count     = local.enable_datadog ? 1 : 0
  secret_id = data.aws_secretsmanager_secret.dd-api-key[0].id
}
