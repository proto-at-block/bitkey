...
resource "aws_iam_policy" "firmware_bucket_get_access_policy" {
  ...
  name        = local.is_developer_stack ? "${local.resource_prefix}-firmware-bucket-get-policy" : "firmware_bucket_get_access_policy"
...
resource "aws_iam_policy" "firmware_bucket_put_access_policy" {
  # Keep old name for shared environments (backward compatibility)
  name        = local.is_developer_stack ? "${local.resource_prefix}-firmware-bucket-put-policy" : "firmware_bucket_put_access_policy"
  description = "Policy to allow access to the firmware bucket"
