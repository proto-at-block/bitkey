...
variable "allow_vpn_ingress" {
  type        = bool
  description = "Allow ingress from Bitkey VPN ingress IPs"
  default     = false
}

variable "vpn_cidr_blocks" {
  type        = list(string)
  description = "List of VPN ingress IPs to allow ingress from"
  default     = ["100.127.228.0/24", "100.127.229.0/24"]
}

################################################################################
# Everything below is from https://github.com/terraform-aws-modules/terraform-aws-rds-aurora/blob/master/variables.tf
################################################################################
