...
commander util gencert --cert-type secureboot --cert-version 1 --cert-pubkey $PRODUCT-bl-signing-key-$KEY_TYPE.pub.pem --sign $PRODUCT-root-firmware-signing-ca-key-$KEY_TYPE.priv.pem --outfile $PRODUCT-bl-signing-cert-$KEY_TYPE.bin
...
commander util gencert --cert-type secureboot --cert-version 1 --cert-pubkey $PRODUCT-app-signing-key-$KEY_TYPE.pub.pem --sign $PRODUCT-bl-signing-key-$KEY_TYPE.priv.pem --outfile $PRODUCT-app-signing-cert-$KEY_TYPE.bin
