...
uint32_t SystemHFRCODPLLClockGet(void) {
  ...
  switch ((HFRCO0->CAL & _HFRCO_CAL_FREQRANGE_MASK) >> _HFRCO_CAL_FREQRANGE_SHIFT) {
    case 0:
...
uint32_t SystemHCLKGet(void) {
  ...
  presc = (CMU->SYSCLKCTRL & _CMU_SYSCLKCTRL_HCLKPRESC_MASK) >> _CMU_SYSCLKCTRL_HCLKPRESC_SHIFT;
...
void SystemHFXOClockSet(uint32_t freq) {
  ...
  if ((CMU->SYSCLKCTRL & _CMU_SYSCLKCTRL_CLKSEL_MASK) == _CMU_SYSCLKCTRL_CLKSEL_HFXO) {
...
uint32_t SystemHFRCOEM23ClockGet(void) {
  ...
  /* Get oscillator frequency band */
  switch ((HFRCOEM23->CAL & _HFRCO_CAL_FREQRANGE_MASK) >> _HFRCO_CAL_FREQRANGE_SHIFT) {
    case 0:
