...
def get_variable_references(elf_file_path: str, variable_name: str) -> list[int]:
    ...
    with open(elf_file_path, "rb") as file:
        ...
        if variable_address is None:
            raise ValueError(
                f"Variable '{variable_name}' not found in the symbol table"
            )
    ...
    with open(elf_file_path, "rb") as file:
        ...
        for cu in dwarf_info.iter_CUs():
            for die_info in cu.iter_DIEs():
                try:
                    if die_info.tag == "DW_TAG_subprogram":
                        ...
                        function_high_address = (
                            function_low_address
                            + die_info.attributes["DW_AT_high_pc"].value
                        )
                        functions.append(
                            {
                                "name": function_name,
            ...
            sorted_functions = sorted(
                functions, key=lambda x: int(x["low_address"], 16)
            )
...
def _find_addr_in_function(functions: list[dict], address: int) -> str:
    ...
    for function in functions:
        if (
            int(function["low_address"], 16)
            <= address
            < int(function["high_address"], 16)
        ):
...
def get_functions_with_variable(
    elf_file_path: str, variable_name: str
) -> dict[str, list]:
    """
        ...
        for compile_unit in dwarf_info.iter_CUs():
            ...
            for entry in line_program.header.file_entry:
                ...
                if dir_index > 0:
                    ...
                    directory = line_program.header.include_directory[
                        dir_index - 1
                    ].decode("utf-8")
