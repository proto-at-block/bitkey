...
class SymbolChecks(bitkey_fwa.TestCase):
    ...
    def fwtest_elf_check_global_symbols_required(self):
        ...
        for sym in symbols:
            self.assertIsNotNone(self.get_elf_symbol(sym), f"symbol not found: {sym}")

    @bitkey_fwa.product("w1a", "w3a-core")
    @bitkey_fwa.suffix("elf")
    def fwtest_elf_check_efr32_symbols_required(self):
        """ "Ensure that EFR32-specific symbols exist where required"""
    ...
    def fwtest_elf_check_symbols_denylist(self):
        """ "Check that various symbols do NOT exist in production non-mfgtest images"""
        ...
        for needle in match_partial:
            self.assertEqual(
                self.get_elf_num_symbols_with_substring(needle),
                0,
        ...
        for sym in match_whole:
            self.assertEqual(
                self.get_elf_num_symbols_with_match(sym),
                0,
    ...
    def fwtest_elf_verify_symbol_pair_empty(self):
        ...
        This ensures that certain arrays are empty for production non-mfgtest images
        """
        ...
        for sym1, sym2 in symbol_pairs:
            self.assertEqual(
                self.get_elf_symbol_value_from_name(sym1),
                self.get_elf_symbol_value_from_name(sym2),
                f"there is data between: {sym1}, {sym2}",
            )
    ...
    def fwtest_elf_verify_symbol_pair_data(self):
        ...
        symbol_pairs = [
            ...
            ("__shared_task_protected_start__", "__shared_task_protected_end__", 32),
        ]
    ...
    @bitkey_fwa.suffix("elf")
    @bitkey_fwa.asset("app")
    @bitkey_fwa.product("w3a-core")
    def fwtest_elf_verify_symbol_pair_data_w3_core(self):
        """Verify MPU task data/bss regions exist and are aligned for w3a-core"""
        ...
        symbol_pairs = [
            ...
            ("__nfc_task_data_start__", "__nfc_task_data_end__", 32),
            ("__ui_task_data_start__", "__ui_task_data_end__", 32),
            ("__usart_task_data_start__", "__usart_task_data_end__", 32),
            ("__nfc_task_bss_start__", "__nfc_task_bss_end__", 32),
            ("__fwup_task_bss_start__", "__fwup_task_bss_end__", 32),
            ("__shared_task_bss_start__", "__shared_task_bss_end__", 32),
            ("__shared_task_protected_start__", "__shared_task_protected_end__", 32),
    ...
    @bitkey_fwa.suffix("elf")
    @bitkey_fwa.asset("app")
    @bitkey_fwa.product("w3a-uxc")
    def fwtest_elf_verify_symbol_pair_data_w3_uxc(self):
        """Verify MPU task data/bss regions exist and are aligned for w3a-uxc"""

        symbol_pairs = [
            ("__display_task_data_start__", "__display_task_data_end__", 32),
            ("__usart_task_data_start__", "__usart_task_data_end__", 32),
            ("__fwup_task_data_start__", "__fwup_task_data_end__", 32),
            ("__shared_task_data_start__", "__shared_task_data_end__", 32),
            ("__fwup_task_bss_start__", "__fwup_task_bss_end__", 32),
    ...
    def fwtest_elf_verify_symbol_alignment(self):
        ...
        for sym, alignment in symbol_alignment_pairs:
            self.assertEqual(
                self.get_elf_symbol_value_from_name(sym) % alignment,
                0,
    ...
    @bitkey_fwa.suffix("elf")
    @bitkey_fwa.asset("app")
    def fwtest_elf_verify_symbol_alignment_stm32u5(self):
        """Verify STM32U5 vector table alignment"""
    ...
    def fwtest_elf_verify_ramfuncs_in_ram(self):
        ...
        self.assertNotEqual(ramfuncs_start, ramfuncs_end, "ramfuncs is empty")

        for func in ramfuncs:
            func_addr = self.get_elf_symbol_value_from_name(func)
            self.assertTrue(
                ramfuncs_start <= func_addr < ramfuncs_end,
        ...
        self.assertTrue(
            data_start <= ramfuncs_start < ramfuncs_end <= data_start + data_size,
