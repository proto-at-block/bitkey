...
static wca_sim_proto_handler_t proto_handler = NULL;
...
static bool handle_proto_exchange(uint8_t* rsp, uint32_t* rsp_len) {
  ...
  if (proto_handler) {
    // Use registered hook for POSIX builds
    uint32_t rsp_size = sizeof(wca_priv.encoded_proto_rsp_ctx.buffer);
    bool ok = proto_handler(
      wca_priv.encoded_proto_cmd_ctx.tag, wca_priv.encoded_proto_cmd_ctx.buffer,
      wca_priv.encoded_proto_cmd_ctx.size, wca_priv.encoded_proto_rsp_ctx.buffer, &rsp_size);
    if (ok && rsp_size > 0) {
      wca_priv.encoded_proto_rsp_ctx.size = rsp_size;
      wca_priv.encoded_proto_rsp_ctx.offset = 0;
  ...
  return true;  // No handler registered, return empty success
#endif
  ...
  bool status =
    ipc_proto_route(wca_priv.encoded_proto_cmd_ctx.tag, wca_priv.encoded_proto_cmd_ctx.buffer,
                    wca_priv.encoded_proto_cmd_ctx.size, seq);
  if (!status) {
