...
// Note: This protocol was written with the idea that we'd support both
// transactio verification and fingerprint reset. We chose to use a different
// protocol for transaction verification, to reduce the number of hardware taps
// in the overall user flow.

// Represents the action requested by the firmware that requires server
// authorization.
typedef enum PACKED {
  ACTION_INVALID,
...
// Structure created by the firmware to request authorization for a sensitive
// action. This structure is serialized and signed by the hardware
// authentication key.
typedef struct PACKED {
  ...
  uint8_t signature[GRANT_SIGNATURE_LEN];  // Signature over the preceding fields
                                           // + label, using the hardware
                                           // authentication key.
} grant_request_t;
...
#define APP_BIND_LABEL          "BKAppBind"
#define APP_ROTATE_LABEL        "BKAppRotate"
#define GRANT_REQUEST_LABEL_LEN (sizeof(GRANT_REQUEST_LABEL) - 1)
#define GRANT_LABEL_LEN         (sizeof(GRANT_LABEL) - 1)
#define APP_BIND_LABEL_LEN      (sizeof(APP_BIND_LABEL) - 1)
#define APP_ROTATE_LABEL_LEN    (sizeof(APP_ROTATE_LABEL) - 1)
...
// This structure contains the original serialized request and the server's
// signature. Serialization must be packed.
typedef struct PACKED {
  uint8_t version;                                      // Protocol version.
  uint8_t serialized_request[sizeof(grant_request_t)];  // The original grant_request_t serialized.
  uint8_t app_signature[GRANT_SIGNATURE_LEN];           // App signature over request core
                                                        // + label.
  uint8_t wsm_signature[GRANT_SIGNATURE_LEN];           // WSM Integrity Key signature over
                                                        // version + serialized_request +
                                                        // app_signature + label.
} grant_t;

#define GRANT_EXCLUDING_WSM_SIGNATURE_LEN (offsetof(grant_t, wsm_signature))
#define GRANT_SIGNABLE_LEN                (GRANT_EXCLUDING_WSM_SIGNATURE_LEN + GRANT_LABEL_LEN)
...
typedef enum {
  ...
  GRANT_RESULT_ERROR_REQUEST_MISMATCH,  // Serialized request in grant doesn't
                                        // match original.
  GRANT_RESULT_ERROR_VERSION_MISMATCH,  // Serialized version in grant doesn't
                                        // match original.
  GRANT_RESULT_ERROR_INTERNAL,          // Other internal errors.
  GRANT_RESULT_ERROR_STORAGE,           // Storage errors.
  GRANT_RESULT_ERROR_APP_VERIFICATION,  // App signature verification failed.
  GRANT_RESULT_ERROR_NO_APP_PUBKEY,     // App auth pubkey not provisioned.
  GRANT_RESULT_ERROR_INVALID_ARGUMENT,  // Invalid argument passed to function.
 ...
 * version, the device ID, a freshly generated challenge, and the specified
 * action. It then signs the request using the hardware's authentication key.
 *
 ...
 * @param[out] out_request Pointer to the grant_request_t structure to be
 * filled. Must not be NULL.
...
/**
 * @brief Deletes the outstanding request. Does nothing if there is no
 * outstanding request.
 *
...
/**
 * @brief Provision the app auth pubkey.
 *
 * @param new_pubkey The new app auth pubkey.
 * @return GRANT_RESULT_OK if the app auth pubkey is provisioned successfully.
 */
grant_protocol_result_t grant_protocol_provision_app_auth_pubkey(const uint8_t* new_pubkey);

/**
 * @brief Deletes the app auth pubkey. Does nothing if there is no
 * app auth pubkey.
 *
 * @return GRANT_RESULT_OK if the app auth pubkey is deleted successfully.
 */
