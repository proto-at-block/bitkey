...
#define MENU_ITEM_TOUCH_DEBUG    fwpb_display_menu_item_DISPLAY_MENU_ITEM_TOUCH_DEBUG
#define ALL_MENU_ITEM_COUNT      9
#else
#define ALL_MENU_ITEM_COUNT 5
#endif
...
static const menu_item_data_t all_menu_items[ALL_MENU_ITEM_COUNT] = {
  ...
  {LANGPACK_ID_MENU_TOUCH_DEBUG, &touch, MENU_ITEM_TOUCH_DEBUG},
...
static menu_item_data_t visible_menu_items[ALL_MENU_ITEM_COUNT];
static uint8_t visible_menu_count = 0;
static lv_obj_t* item_containers[ALL_MENU_ITEM_COUNT];
static lv_obj_t* item_icon_circles[ALL_MENU_ITEM_COUNT];
static lv_obj_t* item_icons[ALL_MENU_ITEM_COUNT];
static lv_obj_t* item_labels[ALL_MENU_ITEM_COUNT];
...
static bool is_mfg_menu_item(fwpb_display_menu_item item_id) {
  return (item_id == MENU_ITEM_TOUCH_DEBUG || item_id == MENU_ITEM_TOGGLE_SLEEP ||
...
static bool menu_item_visible(const menu_item_data_t* item, bool show_lock_device,
...
static void menu_item_click_handler(lv_event_t* e) {
  ...
  if (item_idx < visible_menu_count) {
    ...
    fwpb_display_menu_item menu_item = visible_menu_items[item_idx].item_id;
...
static void scroll_to_item(int index, bool animate) {
  if (!scroll_container || index < 0 || index >= visible_menu_count) {
    return;
  }
...
lv_obj_t* screen_menu_init(void* ctx) {
  ...
  bool show_lock_device = true;
  bool show_fingerprints = true;

  if (show_screen && show_screen->which_params == fwpb_display_show_screen_menu_tag) {
    show_lock_device = !show_screen->params.menu.hide_lock_device;
    show_fingerprints = !show_screen->params.menu.hide_fingerprints;
  }

  visible_menu_count = 0;
  for (uint8_t i = 0; i < ALL_MENU_ITEM_COUNT; i++) {
    if (menu_item_visible(&all_menu_items[i], show_lock_device, show_fingerprints)) {
      visible_menu_items[visible_menu_count++] = all_menu_items[i];
    }
  }
  ...
  // Create menu items
  for (uint8_t i = 0; i < visible_menu_count; i++) {
    ...
    uint32_t circle_color =
      is_mfg_menu_item(visible_menu_items[i].item_id) ? ICON_CIRCLE_COLOR_MFG : ICON_CIRCLE_COLOR;
    ...
    lv_img_set_src(item_icons[i], visible_menu_items[i].icon);
...
#ifdef MFGTEST
    if (visible_menu_items[i].item_id == MENU_ITEM_TOGGLE_SLEEP) {
      lv_label_set_text(item_labels[i], langpack_get_string(get_sleep_toggle_label()));
    } else {
      lv_label_set_text(item_labels[i], langpack_get_string(visible_menu_items[i].label_id));
    }
#else
    lv_label_set_text(item_labels[i], langpack_get_string(visible_menu_items[i].label_id));
#endif
  ...
  if (show_screen && show_screen->which_params == fwpb_display_show_screen_menu_tag) {
    ...
    for (int i = 0; i < visible_menu_count; i++) {
      if (visible_menu_items[i].item_id == selected) {
        initial_item = i;
        break;
      }
    }
...
void screen_menu_destroy(void) {
  ...
  visible_menu_count = 0;
  for (int i = 0; i < ALL_MENU_ITEM_COUNT; i++) {
...
static bool menu_item_visible(const menu_item_data_t* item, bool show_lock_device,
  ...
  if (item->item_id == MENU_ITEM_LOCK_DEVICE) {
    return show_lock_device;
  }

  if (item->item_id == MENU_ITEM_FINGERPRINTS) {
    return show_fingerprints;
  }
