...
void top_back_create(lv_obj_t* parent, top_back_t* button, lv_event_cb_t custom_handler) {
  ...
  lv_obj_clear_flag(button->container, LV_OBJ_FLAG_PRESS_LOCK);
  ...
  lv_obj_set_style_img_recolor(button->icon_img, lv_color_white(), 0);
  lv_obj_set_style_img_recolor_opa(button->icon_img, LV_OPA_COVER, 0);
