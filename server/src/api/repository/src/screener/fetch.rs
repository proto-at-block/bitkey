...
use types::{account::identifiers::AccountId, screener::ScreenerRow};

use super::ScreenerRepository;

impl ScreenerRepository {
    #[instrument(skip(self))]
    pub async fn test_fetch_for_account_id(
        &self,
        account_id: &AccountId,
    ) -> Result<Vec<ScreenerRow>, DatabaseError> {
        ...
        loop {
            let item_output = self.get_connection().client
                ...
                .map_err(|err| {
                    ...
                    event!(
                        Level::ERROR,
                        "Could not fetch screener rows for account id: {account_id} with err: {service_err:?} and message: {:?}",
                        service_err.message(),
                    );
            ...
            let mut consents: Vec<ScreenerRow> = try_from_items(items.to_owned(), database_object)?;
