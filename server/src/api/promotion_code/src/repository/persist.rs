...
use super::PromotionCodeRepository;
use crate::entities::Code;

impl PromotionCodeRepository {
    #[instrument(skip(self))]
    pub async fn persist(&self, code: &Code) -> Result<(), DatabaseError> {
        let table_name = self.get_table_name().await?;
        let database_object = self.get_database_object();
        let item = try_to_item(code, database_object)?;
        let formatted_updated_at = code.updated_at.format(&Rfc3339).map_err(|err| {
        ...
        self.connection
            ...
            .map_err(|err| {
                ...
                event!(
                    Level::ERROR,
                    "Could not persist promotion code: {service_err:?} with message: {:?}",
                    service_err.message()
                );
