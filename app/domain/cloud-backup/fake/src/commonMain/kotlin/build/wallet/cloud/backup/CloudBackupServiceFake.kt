...
class CloudBackupServiceFake : CloudBackupService {
  ...
  private val legacyKeyPrefix = "cloud-backup"
  ...
  override suspend fun readBackup(
    ...
    val backupForRequestedAccount = backups.value[cloudStoreAccount]
      ?.values
      ?.firstOrNull { it.accountId == accountId.serverId }
    if (backupForRequestedAccount != null) {
      return Ok(backupForRequestedAccount)
    }

    // Fall back to legacy/single-backup behavior represented by active backup.
    return Ok(activeBackups[cloudStoreAccount])
  }
    ...
    backups.update {
      it.toMutableMap().apply {
        this[cloudStoreAccount] = (this[cloudStoreAccount] ?: emptyMap()).toMutableMap().apply {
          put("$legacyKeyPrefix-$archiveKey", backup)
        }
        archiveKey++
      }
    }
...
suspend fun CloudBackupServiceFake.awaitNoBackups() {
  backups.test {
...
suspend fun CloudBackupServiceFake.awaitBackup(cloudStoreAccount: CloudStoreAccount): CloudBackup {
  backups.test { awaitUntil { it[cloudStoreAccount] != null } }
  return readActiveBackup(cloudStoreAccount)
    .shouldBeOk().shouldNotBeNull()
}

suspend fun CloudBackupServiceFake.awaitAccountIdMismatchedBackup(
  cloudStoreAccount: CloudStoreAccount,
) {
  backups.test { awaitUntil { it[cloudStoreAccount] != null } }
