...
class MigrationServiceImpl(
    ...
    return coroutineBinding {
      ...
      when (type) {
        ...
        MigrationType.W3Upgrade -> {
          ...
          when {
            // If the migration row was cleared but the account already has W3 hardware,
            // the upgrade already completed — short-circuit to Completed.
            entity == null && account.keybox.config.hardwareType == HardwareType.W3 ->
              MigrationProgress.Completed(type)
            entity?.newHardwareKey == null -> MigrationProgress.NotStarted(type)
            keyset == null -> MigrationProgress.NotStarted(type)
            ...
            !entity.descriptorBackupCompleted ->
              MigrationProgress.DescriptorBackup(type, account.keybox, keyset)
  ...
  private suspend fun handleCreateNewKeyset(
    ...
    return coroutineBinding {
      ...
      state.next(newKeyset = newKeyset, updatedKeybox = updatedKeybox)
    }
  }
    ...
    return coroutineBinding {
      ...
      state.ssek?.let { ssek ->
        val sealedSsek = ensureNotNull(state.sealedSsek) {
          MigrationError.MissingContext("Descriptor backup requires sealedSsek when ssek is present.")
        }
      ...
      val descriptorsToDecrypt: List<bitkey.backup.DescriptorBackup>
      ...
      val sealedSsekForEncryption = when (state.type) {
        ...
        MigrationType.PrivateWalletMigration ->
          ensureNotNull(state.sealedSsek) {
            MigrationError.MissingContext("Descriptor backup requires sealedSsek.")
          }
      }
      val proof = when (state.type) {
        MigrationType.W3Upgrade ->
          ensureNotNull(state.proof) {
            MigrationError.MissingContext("Descriptor backup requires W3 action proof.")
          }
        MigrationType.PrivateWalletMigration ->
          ensureNotNull(state.hwProofOfPossession?.let(PrivilegedActionProof::HwKeyProof)) {
            MigrationError.MissingContext("Descriptor backup requires hardware proof of possession.")
          }
      }
  ...
  private suspend fun handleServerKeysetActivation(
    ...
    return coroutineBinding {
      ...
      val proof = when (state.type) {
        MigrationType.W3Upgrade ->
          ensureNotNull(state.proof) {
            MigrationError.MissingContext("Keyset activation requires W3 action proof.")
          }
        MigrationType.PrivateWalletMigration ->
          ensureNotNull(state.hwProofOfPossession?.let(PrivilegedActionProof::HwKeyProof)) {
            MigrationError.MissingContext("Keyset activation requires hardware proof of possession.")
          }
      }
        ...
        MigrationType.W3Upgrade -> {
          ...
          state.next(
            signedKeysResponse = ensureNotNull(signedKeysResponse) {
              MigrationError.MissingContext("W3 upgrade keyset activation requires signed keyset verification data")
            }
          )
        }
        MigrationType.PrivateWalletMigration -> {
   ...
   * normal rotation with the caller-supplied keys.  This prevents the UI's
   * always-regenerated keys from overwriting previously accepted keys and
   * leaving the migration stuck.
   */
  private suspend fun resolveServerAuthRotation(
    ...
    coroutineBinding {
      ...
      if (!serverAlreadyRotated) {
        ...
        // The UI always regenerates fresh keys on resume, so state.newAppAuthKeys
        // may differ from what the server actually accepted.
        val priorKeys = recoverPersistedAuthKeys(entity)
        if (priorKeys != null) {
          val priorKeyResult = accountAuthenticator.appAuth(
            appAuthPublicKey = priorKeys.newAppAuthKeys.appGlobalAuthPublicKey,
            authTokenScope = Global
          )
          when {
            priorKeyResult.isOk -> {
              logInfo { "Previously persisted auth keys already active on server (crash recovery)" }
              w3UpgradeDao.setServerAuthRotationCompleted()
                .mapError { MigrationError.StatePersistenceFailed(it) }
                .bind()
              return@coroutineBinding priorKeys
            }
            priorKeyResult.error is AuthSignatureMismatch -> {
              // Key definitively not recognized by server — safe to overwrite with fresh keys.
              logInfo { "Persisted auth keys not recognized by server, proceeding with fresh rotation" }
            }
            else -> {
              // Transient/network failure — inconclusive.  Fail fast to preserve the
              // persisted keys; overwriting them with regenerated ones could destroy
              // the only copy of server-accepted key material.
              logInfo { "Prior-key check failed with transient error: ${priorKeyResult.error}" }
              priorKeyResult
        ...
        val proof = state.proof
          ?: error("Auth key rotation requires proof. Populate via withProof() first.")
        when (proof) {
          is PrivilegedActionProof.HwKeyProof -> Unit
          else -> error("W3 upgrade auth rotation requires HwKeyProof proof type")
        }

        val newAppAuthKeys = state.newAppAuthKeys
          ?: error("Auth key rotation requires newAppAuthKeys.")
        val hwAuthPublicKey = state.hwAuthPublicKey
          ?: error("Auth key rotation requires hwAuthPublicKey.")
        val hwSignedAccountId = state.hwSignedAccountId
          ?: error("Auth key rotation requires signed rotation data. Populate via withRotationData() first.")
        ...
        if (serverRotateResult.isOk) {
          ...
          logInfo { "Server rotate call failed, validating whether new keys are already active" }
          val newKeysValid = accountAuthenticator.appAuth(
          ...
          logInfo { "New auth keys already active on server (crash recovery)" }
        }
        ...
        recoverPersistedAuthKeys(entity)
          ?: error("Server rotation completed but persisted auth rotation keys are missing")
      }
    }
    ...
    return coroutineBinding {
      ...
      when (state.type) {
        ...
        MigrationType.W3Upgrade -> {
          ...
          logInfo { "Auth keys rotated locally in keybox" }
          ...
          logInfo { "Auth tokens refreshed for rotated auth keys" }
  ...
  private suspend fun rotateAuthTokens(
    account: FullAccount,
    newAppAuthKeys: AppAuthPublicKeys,
    ...
    coroutineBinding {
      ...
      // Snapshot the current Global tokens so we can restore them if the
      // Recovery write fails.  This avoids a partially-rotated token state
      // where Global uses new keys but Recovery still has stale tokens.
      val previousGlobalTokens = authTokensService
        .getTokens(account.accountId, Global)
        .bind()
      ...
      authTokensService
        ...
        .onFailure {
          // Best-effort rollback: restore the previous Global tokens so both
          // scopes stay consistent.  We use setTokens (not clear()) because
          // clear() is intentionally blocked in Customer builds.
          if (previousGlobalTokens != null) {
            authTokensService.setTokens(account.accountId, previousGlobalTokens, Global)
          }
        }
        .bind()
    }
