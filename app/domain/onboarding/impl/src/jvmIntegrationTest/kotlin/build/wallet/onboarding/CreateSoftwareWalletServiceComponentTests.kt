...
class CreateSoftwareWalletServiceComponentTests : FunSpec({
  ...
  context("happy path") {
    test("successfully create software account") {
      ...
      app.onboardSoftwareAccountService.createAccount().shouldBeOkOfType<SoftwareAccount>()
    }
  ...
  context("unhappy path") {
    test("workflow fails when an account already exists") {
      ...
      app.onboardSoftwareAccountService.createAccount().shouldBeErrOfType<Error>()
    }
