...
use crate::{
    errors::CommandError,
    fwpb::{RotateAppAuthKeysCmd, Status},
...
#[derive(Debug, Clone)]
pub enum RotateAppAuthKeysResult {
    ConfirmationPending {
...
/// Sends rotate app auth keys command to firmware.
...
/// RotateAppAuthKeysRsp with all signatures.
#[generator(yield(Vec<u8>), resume(Vec<u8>))]
fn rotate_app_auth_keys(
...
) -> Result<RotateAppAuthKeysResult, CommandError> {
    let apdu: apdu::Command = RotateAppAuthKeysCmd {
    ...
    if Status::try_from(wallet_rsp.status) == Ok(Status::ConfirmationPending) {
        return Ok(RotateAppAuthKeysResult::ConfirmationPending {
...
command!(RotateAppAuthKeys = rotate_app_auth_keys -> RotateAppAuthKeysResult,
    action_proof_version: u32,
    action: String,
    value: Option<String>,
    bindings: String,
...
mod tests {
    ...
    use super::{RotateAppAuthKeys, RotateAppAuthKeysResult};
    ...
    #[test]
    fn rotate_app_auth_keys_confirmation_pending() -> Result<(), CommandError> {
        let command = RotateAppAuthKeys::new(
            1,
            "RotateAppAuthKeys".to_string(),
            None,
        ...
        match command.next(response) {
            Ok(State::Result {
                value:
                    RotateAppAuthKeysResult::ConfirmationPending {
                        response_handle: rh,
