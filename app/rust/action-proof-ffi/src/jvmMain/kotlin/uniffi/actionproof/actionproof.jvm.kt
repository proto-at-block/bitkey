...
internal object IntegrityCheckingUniffiLib : Library {
    ...
    private fun uniffiCheckApiChecksums() {
        if (uniffi_actionproof_checksum_func_build_payload() != 38130.toShort()) {
...
public object FfiConverterTypeActionProofError : FfiConverterRustBuffer<ActionProofException> {
    override fun read(buf: ByteBuffer): ActionProofException {
        return when (buf.getInt()) {
            ...
            11 -> ActionProofException.BindingsNotSorted(FfiConverterString.read(buf))
...
public actual fun `buildPayload`(`action`: Action, `value`: kotlin.String?, `bindings`: List<ContextBindingPair>): List<kotlin.UByte> {
