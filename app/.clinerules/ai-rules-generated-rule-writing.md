...
- Most rules are automatically included via the index (`@ai-rules/bitkey-mobile.md`).
...
*CLAUDE.md:* Update `CLAUDE.md` with direct rule references

*AGENTS.md*: Update `AGENTS.md` with direct rule references

*goosehints.md:* Update `goosehints.md` with direct rule references. **CRITICAL:** Goose does not understand @ notation and requires explicit relative file paths with contextual descriptions for rule discovery and application.
...
3. Update AI assistant configurations:
   - **CLAUDE.md**: Update `CLAUDE.md` with direct rule references
   - **AGENTS.md**: Update `AGENTS.md` with direct rule references
   - **goosehints.md**:U pdate `goosehints.md` with direct rule references. **CRITICAL:** Goose does not understand @ notation and requires explicit relative file paths with contextual descriptions for rule discovery and application.
4. Verify all references point to existing rules
5. Test rule with multiple AI assistants

**Consistency Verification:**
- For Index/Always-Apply Rules: Create/update/remove `.md` file → update `CLAUDE.md` → update `AGENTS.md` → update `.goosehints.md` 
- Audit: `ls ai-rules/*.md`, `grep @ai-rules CLAUDE.md`,  `grep @ai-rules AGENTS.md`, `grep ai-rules .goosehints.md`
