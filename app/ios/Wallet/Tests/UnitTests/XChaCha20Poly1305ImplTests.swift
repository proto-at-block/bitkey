...
class XChaCha20Poly1305ImplTests: XCTestCase {
    ...
    func test_encryption_without_aad() throws {
        ...
        let ciphertextWithMetadata = try cipher.encrypt(
            key: key,
            nonce: nonce,
            plaintext: plaintext
        )
    ...
    func test_encryption_with_aad() throws {
        ...
        let decrypted = try cipher.decrypt(
            key: key,
            ciphertextWithMetadata: ciphertextWithMetadata,
            aad: aad
        )

        XCTAssertEqual(plaintext, decrypted)
        XCTAssertThrowsError(try cipher.decrypt(
            key: key,
            ciphertextWithMetadata: ciphertextWithMetadata,
            aad: wrongAad
        )) { error in
            XCTAssertEqual(
                error as! core.ChaCha20Poly1305Error,
                core.ChaCha20Poly1305Error.DecryptError(message: "Failed to decrypt")
            )
        }
        XCTAssertThrowsError(try cipher.decrypt(
            key: key,
    ...
    func test_authentication() throws {
        ...
        XCTAssertThrowsError(try cipher.decrypt(
            key: key,
            ciphertextWithMetadata: modifiedSealedData.toOpaqueCiphertext()
        )) { error in
            XCTAssertEqual(
                error as! core.ChaCha20Poly1305Error,
                core.ChaCha20Poly1305Error.DecryptError(message: "Failed to decrypt")
    ...
    func test_rfc_vector() throws {
        ...
        let key = OkioByteString.Companion.shared
            .decodeHex("808182838485868788898a8b8c8d8e8f909192939495969798999a9b9c9d9e9f")
        let iv = OkioByteString.Companion.shared
            .decodeHex("404142434445464748494a4b4c4d4e4f5051525354555657")
        ...
        let expectedTag = OkioByteString.Companion.shared
            .decodeHex("c0875924c1c7987947deafd8780acf49")
        let cipher = XChaCha20Poly1305Impl()
