...
class UNAuthorizationStatusExtensionsTests: XCTestCase {
    ...
    func test_authroized() {
        let status = UNAuthorizationStatusExtensions().convertToNotificationPermissionStatus(
            authorizationStatus:  KotlinLong(integerLiteral: UNAuthorizationStatus.authorized.rawValue)
        )
    ...
    func test_not_determined() {
        let status = UNAuthorizationStatusExtensions().convertToNotificationPermissionStatus(
            authorizationStatus:  KotlinLong(integerLiteral: UNAuthorizationStatus.notDetermined.rawValue)
        )
    ...
    func test_not_determined_ephemeral() {
        let status = UNAuthorizationStatusExtensions().convertToNotificationPermissionStatus(
            authorizationStatus:  KotlinLong(integerLiteral: UNAuthorizationStatus.ephemeral.rawValue)
        )
        XCTAssertEqual(status, PermissionStatus.notdetermined)
    }
    ...
    func test_not_determined_ephemeral_provisional() {
        let status = UNAuthorizationStatusExtensions().convertToNotificationPermissionStatus(
            authorizationStatus:  KotlinLong(integerLiteral: UNAuthorizationStatus.provisional.rawValue)
        )
        XCTAssertEqual(status, PermissionStatus.notdetermined)
    }
