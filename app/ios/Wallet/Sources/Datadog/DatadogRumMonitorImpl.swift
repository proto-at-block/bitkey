...
public final class DatadogRumMonitorImpl: DatadogRumMonitor {
    public func addError(message: String, source: ErrorSource, attributes: [String : String] = [:]) {
         RUMMonitor.shared().addError(message: message, source: source.rumErrorSource, attributes: attributes)
    }
    ...
    public func startResourceLoading(resourceKey: String, method: String, url: String, attributes: [String : String] = [:]) {
        RUMMonitor.shared().startResource(resourceKey: resourceKey, httpMethod: RUMMethod(rawValue: method) ?? .get, urlString: url, attributes: attributes)
    }
    ...
    public func stopResourceLoading(resourceKey: String, kind: ResourceType, attributes: [String : String] = [:]) {
         RUMMonitor.shared().stopResource(resourceKey: resourceKey, statusCode: nil, kind: kind.rumResourceKind, attributes: attributes)
    }

    public func stopResourceLoadingError(resourceKey: String, source: ErrorSource, cause: KotlinThrowable, attributes: [String : String] = [:]) {
         RUMMonitor.shared().stopResourceWithError(resourceKey: resourceKey, error: cause.asError(), attributes: attributes)
    }
