...
public final class Spake2Impl: Shared.Spake2 {
    ...
    public func generateKeyPair(spake2Params: Spake2Params) throws -> Spake2KeyPair {
        let ctx = try core.Spake2Context(
            myRole: toCore(role: spake2Params.role),
            myName: spake2Params.myName,
            theirName: spake2Params.theirName
        )
        ...
        return Spake2KeyPair(
            privateKey: Spake2PrivateKey(bytes: privateKeyByteString),
            publicKey: Spake2PublicKey(bytes: publicKeyByteString)
        )
    }

    public func processTheirPublicKey(
        spake2Params: Spake2Params,
        myKeyPair: Spake2KeyPair,
        theirPublicKey: Spake2PublicKey,
        aad: OkioByteString?
    ) throws -> Spake2SymmetricKeys {
        let ctx = try core.Spake2Context(
            myRole: toCore(role: spake2Params.role),
            myName: spake2Params.myName,
            theirName: spake2Params.theirName
        ...
        try ctx.writeKeyPair(
            privateKey: myKeyPair.privateKey.bytes.toData(),
            publicKey: myKeyPair.publicKey.bytes.toData()
        )
    ...
    public func generateKeyConfMsg(
        role: Shared.Spake2Role,
        keys: Spake2SymmetricKeys
    ) throws -> OkioByteString {
        // Name strings aren't used by `generateKeyConfMsg`, so we can set them to alice and bob
        // rather
        // than requiring them as parameters.
        let ctx = try core.Spake2Context(
            myRole: toCore(role: role),
            myName: "alice",
            theirName: "bob"
        )
    ...
    public func processKeyConfMsg(
        role: Shared.Spake2Role,
        receivedKeyConfMsg: OkioByteString,
        keys: Spake2SymmetricKeys
    ) throws {
        // Name strings aren't used by `generateKeyConfMsg`, so we can set them to alice and bob
        // rather
        ...
        try ctx.processKeyConfMsg(
            receivedMac: receivedKeyConfMsg.toData(),
            keys: toCore(keys: keys)
        )
    }
