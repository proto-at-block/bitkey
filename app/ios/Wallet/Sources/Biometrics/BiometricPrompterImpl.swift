...
public final class BiometricPrompterImpl: BiometricPrompter {
    ...
    public func enrollBiometrics(completionHandler: @escaping (
        BiometricsResult<KotlinUnit>?,
        Error?
    ) -> Void) {
        isPrompting = true
        let context = LAContext()
                ...
                if success {
                    ...
                    if let error = authenticationError as? NSError {
                        ...
                        case .authenticationFailed:
                            completionHandler(
                                BiometricsResultErr(error: BiometricError.AuthenticationFailed()),
                                nil
                            )
                        case .biometryNotAvailable:
                            completionHandler(
                                BiometricsResultErr(error: BiometricError.HardwareUnavailable()),
                                nil
                            )
                        case .biometryNotEnrolled:
                            completionHandler(
                                BiometricsResultErr(error: BiometricError.NoBiometricEnrolled()),
                                nil
                            )
                        case .biometryLockout:
                            completionHandler(
                                BiometricsResultErr(error: BiometricError.BiometricsLocked()),
                                nil
                            )
                        default:
                            completionHandler(
    ...
    public func promptForAuth(
        completionHandler: @escaping (BiometricsResult<KotlinUnit>?, Error?)
            -> Void
    ) {
        isPrompting = true
        let context = LAContext()
