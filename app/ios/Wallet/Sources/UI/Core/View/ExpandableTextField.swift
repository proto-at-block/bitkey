...
public class ExpandableTextField : UITextView, UITextViewDelegate {
    ...
    private var accessoryButtonView: ModeledButton = ModeledButton()
    ...
    // The properties here require us to update the "dummy" placeholder label in order to keep them in sync
    ...
    public override func layoutSubviews() {
        ...
        placeholderTextField.frame = CGRectMake(0, self.contentOffset.y, self.frame.width, self.frame.height)
    }
    
    // MARK: - Public Methods

    public func apply(model: UITextFieldModel, onEnteredTextChanged: @escaping (String, NSRange) -> Void) {
        ...
        // Here is where we take advantage of the placeholder display behavior of `UITextField`. Specifically,
        // the placeholder appears if the text field is empty, and disappears if the text field has some
        // value.
        //
        // Since we set the textColor of `placeholderTextField` to `clear`, the user never actually sees the text
        // that gets set.
...
extension ExpandableTextField {
    private func setup() {
        ...
        // Use a UITextField to pose as a "background" that gets shown/hidden depending on whether or not
    ...
    private func setupConstraints() {
        placeholderTextFieldHeightConstraint = placeholderTextField.heightAnchor.constraint(equalToConstant: 0)
    ...
    private func setupAccessoryButtonViewConstraints() {
        // For some reason, the trailing anchor of the UITextView ends up being at the leading margin.
        // Therefore, we offset the constraint by the entire width of the text view, with a margin of 12.
