...
open class ModeledTextField: UITextField, UITextFieldDelegate {
    ...
    public func apply(
        ...
        // Don't use model.placeholderLabelModel.attributedText as-is because we don't want to set
        // the line height
        let attributedPlaceholder = model.placeholderLabelModel.attributedText
        attributedPlaceholder.removeAttribute(
            .baselineOffset,
            range: .init(location: 0, length: attributedPlaceholder.length)
        )
        attributedPlaceholder.removeAttribute(
            .paragraphStyle,
            range: .init(location: 0, length: attributedPlaceholder.length)
        ...
        if attributedText?.string != model.textLabelModel.text {
            ...
            attributedText = NSAttributedString(
                ...
                attributes: [
                    ...
                    .foregroundColor: model.textLabelModel.textColor,
                ]
            )
        }
