//
// DO NOT EDIT.
// This file is generated from design tokens in the wallet/style folder.
// Run `npm run build` to regenerate.
import SwiftUI
import UIKit

extension UIColor {
  // The default background for the app
  public static let background = UIColor(
    light: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1),
    dark: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1)
  )
  // The background when the view has containers
  public static let groupedBackground = UIColor(
    light: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1),
    dark: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1)
  )
  // The default background for a container
  public static let containerBackground = UIColor(
    light: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1),
    dark: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1)
  )
  // The background for a container that needs to draw attention
  public static let containerBackgroundHighlight = UIColor(
    light: UIColor(red: 0.961, green: 0.973, blue: 0.996, alpha: 1),
    dark: UIColor(red: 0.961, green: 0.973, blue: 0.996, alpha: 1)
  )
  // foreground blue used on a highlighted container
  public static let containerHighlightForeground = UIColor(
    light: UIColor(red: 0.647, green: 0.776, blue: 1.000, alpha: 1),
    dark: UIColor(red: 0.647, green: 0.776, blue: 1.000, alpha: 1)
  )
  // The main foreground color for things like text
  public static let foreground = UIColor(
    light: UIColor(red: 0.067, green: 0.067, blue: 0.067, alpha: 1),
    dark: UIColor(red: 0.067, green: 0.067, blue: 0.067, alpha: 1)
  )
  // The secondary foreground color for things like secondary text
  public static let foreground60 = UIColor(
    light: UIColor(red: 0.400, green: 0.400, blue: 0.400, alpha: 1),
    dark: UIColor(red: 0.400, green: 0.400, blue: 0.400, alpha: 1)
  )
  // used for chevrons
  public static let foreground30 = UIColor(
    light: UIColor(red: 0.776, green: 0.776, blue: 0.776, alpha: 1),
    dark: UIColor(red: 0.776, green: 0.776, blue: 0.776, alpha: 1)
  )
  // used for borders and dividers
  public static let foreground10 = UIColor(
    light: UIColor(red: 0.965, green: 0.965, blue: 0.965, alpha: 1),
    dark: UIColor(red: 0.965, green: 0.965, blue: 0.965, alpha: 1)
  )
  // The primary color for buttons, switches, etc
  public static let bitkeyPrimary = UIColor(
    light: UIColor(red: 0.000, green: 0.502, blue: 0.588, alpha: 1),
    dark: UIColor(red: 0.000, green: 0.502, blue: 0.588, alpha: 1)
  )
  // The foreground color for things on top of the primary color like text and icons
  public static let primaryForeground = UIColor(
    light: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1),
    dark: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1)
  )
  // The disabled foreground color for things on top of the primary color like text and icons
  public static let primaryForeground30 = UIColor(
    light: UIColor(red: 0.776, green: 0.776, blue: 0.776, alpha: 1),
    dark: UIColor(red: 0.776, green: 0.776, blue: 0.776, alpha: 1)
  )
  // The background color for secondary buttons
  public static let secondary = UIColor(
    light: UIColor(red: 0.000, green: 0.000, blue: 0.000, alpha: 0.0392156862745098),
    dark: UIColor(red: 0.000, green: 0.000, blue: 0.000, alpha: 0.0392156862745098)
  )
  // Used as the foreground color for things on top of the secondary button like text and icons
  public static let secondaryForeground = UIColor(
    light: UIColor(red: 0.067, green: 0.067, blue: 0.067, alpha: 1),
    dark: UIColor(red: 0.067, green: 0.067, blue: 0.067, alpha: 1)
  )
  // The disabled foreground color for things on top of the secondary button like text and icons
  public static let secondaryForeground30 = UIColor(
    light: UIColor(red: 0.776, green: 0.776, blue: 0.776, alpha: 1),
    dark: UIColor(red: 0.776, green: 0.776, blue: 0.776, alpha: 1)
  )
  // The primary color for icons
  public static let primaryIcon = UIColor(
    light: UIColor(red: 0.067, green: 0.067, blue: 0.067, alpha: 1),
    dark: UIColor(red: 0.067, green: 0.067, blue: 0.067, alpha: 1)
  )
  // The primary foreground color for icons
  public static let primaryIconForeground = UIColor(
    light: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1),
    dark: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1)
  )
  // The secondary color for icons
  public static let secondaryIcon = UIColor(
    light: UIColor(red: 0.000, green: 0.000, blue: 0.000, alpha: 0.050980392156862744),
    dark: UIColor(red: 0.000, green: 0.000, blue: 0.000, alpha: 0.050980392156862744)
  )
  // The secondary foreground color for icons
  public static let secondaryIconForeground = UIColor(
    light: UIColor(red: 0.067, green: 0.067, blue: 0.067, alpha: 1),
    dark: UIColor(red: 0.067, green: 0.067, blue: 0.067, alpha: 1)
  )
  // The background color for normal (20%) translucent buttons
  public static let translucentButton20 = UIColor(
    light: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 0.2),
    dark: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 0.2)
  )
  // The background color for lighter (10%) translucent buttons
  public static let translucentButton10 = UIColor(
    light: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 0.10196078431372549),
    dark: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 0.10196078431372549)
  )
  // The foreground color for translucent buttons
  public static let translucentForeground = UIColor(
    light: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1),
    dark: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1)
  )
  // Mask color
  public static let mask = UIColor(
    light: UIColor(red: 0.067, green: 0.067, blue: 0.067, alpha: 0.6),
    dark: UIColor(red: 0.067, green: 0.067, blue: 0.067, alpha: 0.6)
  )
  // Positive banner foreground color
  public static let positive = UIColor(
    light: UIColor(red: 0.000, green: 0.576, blue: 0.216, alpha: 0.050980392156862744),
    dark: UIColor(red: 0.000, green: 0.576, blue: 0.216, alpha: 0.050980392156862744)
  )
  // Positive color
  public static let positiveForeground = UIColor(
    light: UIColor(red: 0.000, green: 0.502, blue: 0.000, alpha: 1),
    dark: UIColor(red: 0.000, green: 0.502, blue: 0.000, alpha: 1)
  )
  // Destructive color
  public static let destructive = UIColor(
    light: UIColor(red: 1.000, green: 0.000, blue: 0.000, alpha: 1),
    dark: UIColor(red: 1.000, green: 0.000, blue: 0.000, alpha: 1)
  )
  // Destructive foreground color
  public static let destructiveForeground = UIColor(
    light: UIColor(red: 0.792, green: 0.000, blue: 0.000, alpha: 1),
    dark: UIColor(red: 0.792, green: 0.000, blue: 0.000, alpha: 1)
  )
  // Warning banner background color
  public static let warning = UIColor(
    light: UIColor(red: 1.000, green: 0.961, blue: 0.941, alpha: 1),
    dark: UIColor(red: 1.000, green: 0.961, blue: 0.941, alpha: 1)
  )
  // Warning foreground color
  public static let warningForeground = UIColor(
    light: UIColor(red: 0.957, green: 0.431, blue: 0.220, alpha: 1),
    dark: UIColor(red: 0.957, green: 0.431, blue: 0.220, alpha: 1)
  )
  // Danger color
  public static let danger = UIColor(
    light: UIColor(red: 0.882, green: 0.267, blue: 0.145, alpha: 1),
    dark: UIColor(red: 0.882, green: 0.267, blue: 0.145, alpha: 1)
  )
  // Danger background color
  public static let dangerBackground = UIColor(
    light: UIColor(red: 0.882, green: 0.267, blue: 0.145, alpha: 0.10196078431372549),
    dark: UIColor(red: 0.882, green: 0.267, blue: 0.145, alpha: 0.10196078431372549)
  )
  // Device LED green color
  public static let deviceLEDGreen = UIColor(
    light: UIColor(red: 0.000, green: 0.502, blue: 0.000, alpha: 1),
    dark: UIColor(red: 0.000, green: 0.502, blue: 0.000, alpha: 1)
  )
  // Device LED red color
  public static let deviceLEDRed = UIColor(
    light: UIColor(red: 1.000, green: 0.000, blue: 0.000, alpha: 1),
    dark: UIColor(red: 1.000, green: 0.000, blue: 0.000, alpha: 1)
  )
  // Device LED blue color
  public static let deviceLEDBlue = UIColor(
    light: UIColor(red: 0.000, green: 0.349, blue: 1.000, alpha: 1),
    dark: UIColor(red: 0.000, green: 0.349, blue: 1.000, alpha: 1)
  )
  // Device LED white color
  public static let deviceLEDWhite = UIColor(
    light: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1),
    dark: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1)
  )
  // nfc blue
  public static let nfcBlue = UIColor(
    light: UIColor(red: 0.239, green: 0.506, blue: 1.000, alpha: 1),
    dark: UIColor(red: 0.239, green: 0.506, blue: 1.000, alpha: 1)
  )
  // Background color for the initial landing screen before any setup
  public static let bitkeyGetStartedBackground = UIColor(
    light: UIColor(red: 0.125, green: 0.118, blue: 0.133, alpha: 1),
    dark: UIColor(red: 0.125, green: 0.118, blue: 0.133, alpha: 1)
  )
  // Tint color for the logo and text on the initial landing screen before any setup
  public static let bitkeyGetStartedTint = UIColor(
    light: UIColor(red: 0.973, green: 0.957, blue: 0.894, alpha: 1),
    dark: UIColor(red: 0.973, green: 0.957, blue: 0.894, alpha: 1)
  )
  // Color for the out of date icon in the app status screen
  public static let outOfDate = UIColor(
    light: UIColor(red: 0.925, green: 0.663, blue: 0.000, alpha: 1),
    dark: UIColor(red: 0.925, green: 0.663, blue: 0.000, alpha: 1)
  )
  // Default callout title color
  public static let calloutDefaultTitle = UIColor(
    light: UIColor(red: 0.000, green: 0.000, blue: 0.000, alpha: 0.8980392156862745),
    dark: UIColor(red: 0.000, green: 0.000, blue: 0.000, alpha: 0.8980392156862745)
  )
  // Default callout subtitle color
  public static let calloutDefaultSubtitle = UIColor(
    light: UIColor(red: 0.000, green: 0.000, blue: 0.000, alpha: 0.6980392156862745),
    dark: UIColor(red: 0.000, green: 0.000, blue: 0.000, alpha: 0.6980392156862745)
  )
  // Default callout background color
  public static let calloutDefaultBackground = UIColor(
    light: UIColor(red: 0.949, green: 0.949, blue: 0.949, alpha: 1),
    dark: UIColor(red: 0.949, green: 0.949, blue: 0.949, alpha: 1)
  )
  // Default callout title color
  public static let calloutDefaultTrailingIcon = UIColor(
    light: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1),
    dark: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1)
  )
  // Default callout title color
  public static let calloutDefaultTrailingIconBackground = UIColor(
    light: UIColor(red: 0.000, green: 0.000, blue: 0.000, alpha: 1),
    dark: UIColor(red: 0.000, green: 0.000, blue: 0.000, alpha: 1)
  )
  // Information callout title color
  public static let calloutInformationTitle = UIColor(
    light: UIColor(red: 0.000, green: 0.294, blue: 0.451, alpha: 1),
    dark: UIColor(red: 0.000, green: 0.294, blue: 0.451, alpha: 1)
  )
  // Information callout subtitle color
  public static let calloutInformationSubtitle = UIColor(
    light: UIColor(red: 0.000, green: 0.294, blue: 0.451, alpha: 1),
    dark: UIColor(red: 0.000, green: 0.294, blue: 0.451, alpha: 1)
  )
  // Information callout background color
  public static let calloutInformationBackground = UIColor(
    light: UIColor(red: 0.047, green: 0.604, blue: 0.855, alpha: 0.10196078431372549),
    dark: UIColor(red: 0.047, green: 0.604, blue: 0.855, alpha: 0.10196078431372549)
  )
  // Information callout trailing icon color
  public static let calloutInformationTrailingIcon = UIColor(
    light: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1),
    dark: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1)
  )
  // Information callout trailing icon background color
  public static let calloutInformationTrailingIconBackground = UIColor(
    light: UIColor(red: 0.047, green: 0.604, blue: 0.855, alpha: 1),
    dark: UIColor(red: 0.047, green: 0.604, blue: 0.855, alpha: 1)
  )
  // Success callout title color
  public static let calloutSuccessTitle = UIColor(
    light: UIColor(red: 0.043, green: 0.361, blue: 0.122, alpha: 1),
    dark: UIColor(red: 0.043, green: 0.361, blue: 0.122, alpha: 1)
  )
  // Success callout subtitle color
  public static let calloutSuccessSubtitle = UIColor(
    light: UIColor(red: 0.043, green: 0.361, blue: 0.122, alpha: 1),
    dark: UIColor(red: 0.043, green: 0.361, blue: 0.122, alpha: 1)
  )
  // Success callout background color
  public static let calloutSuccessBackground = UIColor(
    light: UIColor(red: 0.565, green: 0.765, blue: 0.000, alpha: 0.10196078431372549),
    dark: UIColor(red: 0.565, green: 0.765, blue: 0.000, alpha: 0.10196078431372549)
  )
  // Success callout trailing icon color
  public static let calloutSuccessTrailingIcon = UIColor(
    light: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1),
    dark: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1)
  )
  // Success callout trailing icon background color
  public static let calloutSuccessTrailingIconBackground = UIColor(
    light: UIColor(red: 0.565, green: 0.765, blue: 0.000, alpha: 1),
    dark: UIColor(red: 0.565, green: 0.765, blue: 0.000, alpha: 1)
  )
  // Warning callout title color
  public static let calloutWarningTitle = UIColor(
    light: UIColor(red: 0.529, green: 0.286, blue: 0.000, alpha: 1),
    dark: UIColor(red: 0.529, green: 0.286, blue: 0.000, alpha: 1)
  )
  // Warning callout subtitle color
  public static let calloutWarningSubtitle = UIColor(
    light: UIColor(red: 0.529, green: 0.286, blue: 0.000, alpha: 1),
    dark: UIColor(red: 0.529, green: 0.286, blue: 0.000, alpha: 1)
  )
  // Warning callout background color
  public static let calloutWarningBackground = UIColor(
    light: UIColor(red: 0.992, green: 0.541, blue: 0.000, alpha: 0.10196078431372549),
    dark: UIColor(red: 0.992, green: 0.541, blue: 0.000, alpha: 0.10196078431372549)
  )
  // Warning callout trailing icon color
  public static let calloutWarningTrailingIcon = UIColor(
    light: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1),
    dark: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1)
  )
  // Warning callout trailing icon background color
  public static let calloutWarningTrailingIconBackground = UIColor(
    light: UIColor(red: 0.992, green: 0.541, blue: 0.000, alpha: 1),
    dark: UIColor(red: 0.992, green: 0.541, blue: 0.000, alpha: 1)
  )
  // Danger callout title color
  public static let calloutDangerTitle = UIColor(
    light: UIColor(red: 0.455, green: 0.078, blue: 0.051, alpha: 1),
    dark: UIColor(red: 0.455, green: 0.078, blue: 0.051, alpha: 1)
  )
  // Danger callout subtitle color
  public static let calloutDangerSubtitle = UIColor(
    light: UIColor(red: 0.455, green: 0.078, blue: 0.051, alpha: 1),
    dark: UIColor(red: 0.455, green: 0.078, blue: 0.051, alpha: 1)
  )
  // Danger callout trailing icon color
  public static let calloutDangerTrailingIcon = UIColor(
    light: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1),
    dark: UIColor(red: 1.000, green: 1.000, blue: 1.000, alpha: 1)
  )
  // Coachmark background color
  public static let coachmarkBackground = UIColor(
    light: UIColor(red: 0.200, green: 0.204, blue: 0.259, alpha: 1),
    dark: UIColor(red: 0.200, green: 0.204, blue: 0.259, alpha: 1)
  )
}

extension Color {
  public static let background = Color(from: .background)
  public static let groupedBackground = Color(from: .groupedBackground)
  public static let containerBackground = Color(from: .containerBackground)
  public static let containerBackgroundHighlight = Color(from: .containerBackgroundHighlight)
  public static let containerHighlightForeground = Color(from: .containerHighlightForeground)
  public static let foreground = Color(from: .foreground)
  public static let foreground60 = Color(from: .foreground60)
  public static let foreground30 = Color(from: .foreground30)
  public static let foreground10 = Color(from: .foreground10)
  public static let bitkeyPrimary = Color(from: .bitkeyPrimary)
  public static let primaryForeground = Color(from: .primaryForeground)
  public static let primaryForeground30 = Color(from: .primaryForeground30)
  public static let secondary = Color(from: .secondary)
  public static let secondaryForeground = Color(from: .secondaryForeground)
  public static let secondaryForeground30 = Color(from: .secondaryForeground30)
  public static let primaryIcon = Color(from: .primaryIcon)
  public static let primaryIconForeground = Color(from: .primaryIconForeground)
  public static let secondaryIcon = Color(from: .secondaryIcon)
  public static let secondaryIconForeground = Color(from: .secondaryIconForeground)
  public static let translucentButton20 = Color(from: .translucentButton20)
  public static let translucentButton10 = Color(from: .translucentButton10)
  public static let translucentForeground = Color(from: .translucentForeground)
  public static let mask = Color(from: .mask)
  public static let positive = Color(from: .positive)
  public static let positiveForeground = Color(from: .positiveForeground)
  public static let destructive = Color(from: .destructive)
  public static let destructiveForeground = Color(from: .destructiveForeground)
  public static let warning = Color(from: .warning)
  public static let warningForeground = Color(from: .warningForeground)
  public static let danger = Color(from: .danger)
  public static let dangerBackground = Color(from: .dangerBackground)
  public static let deviceLEDGreen = Color(from: .deviceLEDGreen)
  public static let deviceLEDRed = Color(from: .deviceLEDRed)
  public static let deviceLEDBlue = Color(from: .deviceLEDBlue)
  public static let deviceLEDWhite = Color(from: .deviceLEDWhite)
  public static let nfcBlue = Color(from: .nfcBlue)
  public static let bitkeyGetStartedBackground = Color(from: .bitkeyGetStartedBackground)
  public static let bitkeyGetStartedTint = Color(from: .bitkeyGetStartedTint)
  public static let outOfDate = Color(from: .outOfDate)
  public static let calloutDefaultTitle = Color(from: .calloutDefaultTitle)
  public static let calloutDefaultSubtitle = Color(from: .calloutDefaultSubtitle)
  public static let calloutDefaultBackground = Color(from: .calloutDefaultBackground)
  public static let calloutDefaultTrailingIcon = Color(from: .calloutDefaultTrailingIcon)
  public static let calloutDefaultTrailingIconBackground = Color(from: .calloutDefaultTrailingIconBackground)
  public static let calloutInformationTitle = Color(from: .calloutInformationTitle)
  public static let calloutInformationSubtitle = Color(from: .calloutInformationSubtitle)
  public static let calloutInformationBackground = Color(from: .calloutInformationBackground)
  public static let calloutInformationTrailingIcon = Color(from: .calloutInformationTrailingIcon)
  public static let calloutInformationTrailingIconBackground = Color(from: .calloutInformationTrailingIconBackground)
  public static let calloutSuccessTitle = Color(from: .calloutSuccessTitle)
  public static let calloutSuccessSubtitle = Color(from: .calloutSuccessSubtitle)
  public static let calloutSuccessBackground = Color(from: .calloutSuccessBackground)
  public static let calloutSuccessTrailingIcon = Color(from: .calloutSuccessTrailingIcon)
  public static let calloutSuccessTrailingIconBackground = Color(from: .calloutSuccessTrailingIconBackground)
  public static let calloutWarningTitle = Color(from: .calloutWarningTitle)
  public static let calloutWarningSubtitle = Color(from: .calloutWarningSubtitle)
  public static let calloutWarningBackground = Color(from: .calloutWarningBackground)
  public static let calloutWarningTrailingIcon = Color(from: .calloutWarningTrailingIcon)
  public static let calloutWarningTrailingIconBackground = Color(from: .calloutWarningTrailingIconBackground)
  public static let calloutDangerTitle = Color(from: .calloutDangerTitle)
  public static let calloutDangerSubtitle = Color(from: .calloutDangerSubtitle)
  public static let calloutDangerTrailingIcon = Color(from: .calloutDangerTrailingIcon)
  public static let coachmarkBackground = Color(from: .coachmarkBackground)
}
