...
/**
 * [F8eServiceLogger] is a Ktor Client plugin that allows logging of
...
class F8eServiceLogger private constructor(
  val tag: String,
  val debug: Boolean,
  ...
  companion object : HttpClientPlugin<Config, F8eServiceLogger> {
    override val key: AttributeKey<F8eServiceLogger> = AttributeKey("KtorServiceLogger")

    override fun prepare(block: Config.() -> Unit): F8eServiceLogger {
      val config = Config().apply(block)
      return F8eServiceLogger(
        tag = config.tag,
        debug = config.debug,
