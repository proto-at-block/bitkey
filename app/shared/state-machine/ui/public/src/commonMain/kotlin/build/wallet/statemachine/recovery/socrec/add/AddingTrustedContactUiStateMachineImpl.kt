...
import build.wallet.statemachine.recovery.socrec.add.AddingTrustedContactUiStateMachineImpl.State.InheritanceExplainer
import build.wallet.statemachine.recovery.socrec.add.AddingTrustedContactUiStateMachineImpl.State.InheritanceLearnMore
import build.wallet.statemachine.recovery.socrec.add.AddingTrustedContactUiStateMachineImpl.State.InheritanceSetup
import build.wallet.statemachine.recovery.socrec.add.AddingTrustedContactUiStateMachineImpl.State.LoadingNotificationPermissions
import build.wallet.statemachine.recovery.socrec.add.AddingTrustedContactUiStateMachineImpl.State.NotificationPermissionsLoadError
import build.wallet.statemachine.recovery.socrec.add.AddingTrustedContactUiStateMachineImpl.State.RequestingNotificationPermissions
...
import build.wallet.statemachine.settings.full.notifications.NotificationsService.NotificationStatus
      ...
      is Success -> {
        SuccessBodyModel(
          ...
          primaryButtonModel = ButtonDataModel(
            ...
            onClick = {
              promoCode?.let {
                state = State.ShowingPromoCodeUpsell(promoCode = it, contactAlias = current.contactAlias)
