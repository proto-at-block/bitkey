...
class BitcoinWalletServiceFunctionalTests : FunSpec({
  ...
  test("faster psbts should have all inputs from slower psbts")
    .config(tags = setOf(IsolatedTest)) {
      app.onboardFullAccountWithFakeHardware()

      bitcoinWalletService.transactionsData().test {
        // Add confirmed utxo
        app.addSomeFunds(amount = sats(1_000), waitForConfirmation = true)
        app.addSomeFunds(amount = sats(2_000), waitForConfirmation = true)
        app.addSomeFunds(amount = sats(3_000), waitForConfirmation = true)

        awaitUntil {
          it?.balance?.confirmed == sats(6_000)
        }

        val psbts = bitcoinWalletService.createPsbtsForSendAmount(
