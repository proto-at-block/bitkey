...
class AuthKeyRotationManagerImplTests : FunSpec({
  ...
  test("start new auth key rotation") {
    ...
    val result = authKeyRotationManager.startOrResumeAuthKeyRotation(
      request = request,
  ...
  test("resume auth key rotation") {
    ...
    keyboxDao.rotateKeyboxResult = Ok(
      KeyboxMock.copy(
        ...
        appGlobalAuthKeyHwSignature = request.newKeys.requireAppGlobalAuthKeyHwSignature()
      )
    )
    ...
    bestEffortFullAccountCloudBackupUploader.createAndUploadCloudBackupCalls.awaitItem()
      ...
      .should { backedUpAccount ->
        ...
        backedUpAccount.keybox.appGlobalAuthKeyHwSignature shouldBe request.newKeys.requireAppGlobalAuthKeyHwSignature()
      }
